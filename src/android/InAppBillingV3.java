...
public class InAppBillingV3 extends CordovaPlugin {
    ...
    private JSONObject getManifestContents() {
        ...
        InputStream is;
        try {
            is = context.getAssets().open("www/my-manifest.json");
    ...
    protected String getBase64EncodedPublicKey() {
        ...
        if (manifestObject != null) {
            return manifestObject.optString("jhoobin_store_key");
        }
        return null;
    }
    ...
    protected String getCharkhoneSecretKeyOne() {
        JSONObject manifestObject = getManifestContents();
        if (manifestObject != null) {
            return manifestObject.optString("charkhone_secret_one");
        }
        return null;
    }

    protected String getCharkhoneSecretKeyTwo() {
        JSONObject manifestObject = getManifestContents();
        if (manifestObject != null) {
            return manifestObject.optString("charkhone_secret_two");
        }
        return null;
    }

    protected boolean shouldForceUseSDK() {
        JSONObject manifestObject = getManifestContents();
        if (manifestObject != null) {
            return manifestObject.optBoolean("use_sdk_only");
        }
        return false;
    }
    ...
    protected boolean initializeBillingHelper() {
        ...
        String secretKeyOne = getCharkhoneSecretKeyOne();
        String secretKeyTwo = getCharkhoneSecretKeyTwo();
        if (base64EncodedPublicKey != null && secretKeyOne != null && secretKeyTwo != null) {
            PluginInitialization.initSDK(this.cordova.getActivity(), secretKeyOne, secretKeyTwo, "cordova-ionic");
            iabHelper = new IabHelper(context, base64EncodedPublicKey, new MarketIntentFactorySDK(shouldForceUseSDK()));
    ...
    public boolean execute(String action, final JSONArray args, final CallbackContext callbackContext) {
        Log.d(TAG, "executing on android: " + action);
    ...
    private boolean setMsisdn(JSONArray args, CallbackContext callbackContext) {
        ...
        try {
            ...
            callbackContext.error(makeError("Invalid msisdn", INVALID_ARGUMENTS));
            return false;
        }
        try {
            editAble = args.getBoolean(1);
        } catch (JSONException e) {
            callbackContext.error(makeError("Invalid argument", INVALID_ARGUMENTS));
            return false;
        }

        if (iabHelper != null) {
            Intent fillInIntent = new Intent();
            fillInIntent.putExtra("msisdn", msisdn);
            fillInIntent.putExtra("editAble", editAble);
            iabHelper.addFillInIntent(fillInIntent);
    ...
    private boolean setDelayPayment(JSONArray args, CallbackContext callbackContext) {
        Integer delayPayment;
        try {
            delayPayment = args.getInt(0);
        } catch (JSONException e) {
            callbackContext.error(makeError("Invalid argument", INVALID_ARGUMENTS));
            return false;
        }
        ...
        if (iabHelper != null) {
            Intent fillInIntent = new Intent();
            fillInIntent.putExtra("delayPayment", delayPayment);
            iabHelper.addFillInIntent(fillInIntent);
...
//                if (result.isFailure()) {
//                    callbackContext.error("Error retrieving SKU details" + moreSkus);
//                    return;
//                }
